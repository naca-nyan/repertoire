...
const MyPageContent: React.FC<{
  ...
  const [innerArtists, setInnerArtists] = useState<string[] | undefined>(
    undefined
  );
  const artists = innerArtists ?? [];
  ...
  useEffect(() => {
    return watchOrderArtists(userId, (order) => setInnerArtists(order));
  }, [userId]);
  ...
  const setInitialOrderArtists = () => {
    if (!innerArtists) return;
    const res: Record<string, boolean> = {};
    for (const artist of innerArtists) {
      res[artist] = false;
    }
    ...
    setInnerArtists(Object.keys(res));
  };
  ...
  return (
    <Container maxWidth="xl" sx={{ mt: 3 }}>
          ...
          <ButtonGroup style={{ display: "flex", justifyContent: "flex-end" }}>
            ...
            <Tooltip title="アーティスト名を並び換え">
              <Button
                ...
                disabled={!innerArtists}
                onClick={() => {
                  if (!innerArtists) return;
                  if (sortArtist) setOrderArtists(userId, innerArtists);
                  else setInitialOrderArtists();
      ...
      {sortArtist ? (
        <SortableArtistList artists={artists} onSortEnd={setInnerArtists} />
      ) : (
        <SongList
